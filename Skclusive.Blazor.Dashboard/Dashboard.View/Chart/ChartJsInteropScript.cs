...
namespace Skclusive.Blazor.Dashboard.View.Chart
{
    public class ChartJsInteropScript : StaticComponentBase
    {
...
class ChartJsInterop {
  ...
  HandleDatasets(myChart, config) {
    ...
    let dataSetsToRemove = myChart.config.data.datasets.filter(
      d => config.data.datasets.find(newD => newD.id === d.id) === undefined
    );
    ...
    // Add new datasets
    let dataSetsToAdd = config.data.datasets.filter(
      newD =>
        myChart.config.data.datasets.find(d => newD.id === d.id) === undefined
    );
    ...
    let datasetsToUpdate = myChart.config.data.datasets
      ...
      .map(d => ({
        oldD: d,
        newD: config.data.datasets.find(val => val.id === d.id)
      }));
    datasetsToUpdate.forEach(pair => {
      ...
      Object.entries(pair.newD).forEach(
        entry => (pair.oldD[entry[0]] = entry[1])
      );
    });
  }
  ...
  WireUpLegendItemFilterFunc(config) {
    ...
    if (
      config.options.legend.labels.filter &&
      ...
      const filterFunc =
        window[filtersNamespaceAndFunc[0]][filtersNamespaceAndFunc[1]];
      if (typeof filterFunc === 'function') {
        config.options.legend.labels.filter = filterFunc;
      } else {
        // fallback to the default, which is null
  ...
  WireUpGenerateLabelsFunc(config) {
    ...
    if (config.options.legend.labels === undefined)
      config.options.legend.labels = {};
    if (
      config.options.legend.labels.generateLabels &&
      ...
      const generateLabelsFunc =
        window[generateLabelsNamespaceAndFunc[0]][
          generateLabelsNamespaceAndFunc[1]
        ];
      if (typeof generateLabelsFunc === 'function') {
        config.options.legend.labels.generateLabels = generateLabelsFunc;
      } else {
        // fallback to the default
        config.options.legend.labels.generateLabels = getDefaultFunc(
          config.type
        );
      }
    } else {
  ...
  WireUpOptionsOnClickFunc(config) {
    ...
    config.options.onClick = this.GetHandler(
      config.options.onClick,
      getDefaultFunc(config.type)
    );
  }
  WireUpOptionsOnHoverFunc(config) {
    ...
    if (config.options.hover) {
      config.options.hover.onHover = this.GetHandler(
        config.options.hover.onHover,
        getDefaultFunc(config.type)
      );
    }
  }
  WireUpLegendOnClick(config) {
    ...
    config.options.legend.onClick = this.GetHandler(
      config.options.legend.onClick,
      getDefaultHandler(config.type)
    );
  }
  WireUpLegendOnHover(config) {
    ...
    config.options.legend.onHover = this.GetHandler(
      config.options.legend.onHover,
      getDefaultFunc(config.type)
    );
  }
  /**
  ...
  GetHandler(iClickHandler, chartJsDefaultHandler) {
    if (iClickHandler) {
      ...
      if (
        ...
        const onClickFunc =
          window[onClickNamespaceAndFunc[0]][onClickNamespaceAndFunc[1]];
      ...
      // .Net instance method
        ...
        return (function() {
          ...
          return async function(sender, args) {
            await instanceRef.invokeMethodAsync(
              methodName,
              sender,
...
class MomentJsInterop {
  ...
  changeLocale(locale) {
    ...
    if (cur === newL)
      throw ""the locale '"" +
        locale +
        ""' could not be set. It was probably not found."";
    return true;
  }
}
